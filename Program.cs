...
GetOptionParam(out string imgFile, out Int32 format, out bool toBGR, out Int32 linePixel);
...
Console.WriteLine($"//{Path.GetFileNameWithoutExtension(imgFile)} - {bmp.Width}x{bmp.Height} - " +
	$"{(toBGR ? "BGR" : "RGB")}:{format} ({fmtByteLength:N0}byte)");
...
void Usage()
{
	string mes = @"書式 : ImageToBytes.exe <画像ファイル> [-f<n>] [-b<n>] [-bgr]
		...
		-bgr  : BGR形式で出力する.　省略時はRGB形式.
		例
		ImageToBytes.exe c:\dir\myimage.jpg -f565 -b20 >z:\myimage565.txt".Replace("\t", "");
	mes = string.Format(mes, FormatDefault, LinePixelDefault);
	Console.WriteLine(mes);
}

//オプション解析
void GetOptionParam(out string imgFile, out Int32 format, out bool toBGR, out Int32 linePixel)
{
	...
	foreach (string option in args)
	{
		...
		if (option.ToLower().Equals("-bgr"))
		{
			toBGR = true;
			continue;   //後続の'-b'判定に掛からないようにする
		}
